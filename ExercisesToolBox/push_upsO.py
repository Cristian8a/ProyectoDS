...
with np_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while True:
        ...
        if results.pose_landmarks:
            ...
            cv2.circle(img, rigthWrist, 15, (0,0,255), 15)
            cv2.circle(img, rigthShoulder, 15, (0,0,255), 15)

            # Verificamos si se ha iniciado o finalizado una lagartija.
            if distancia_euc(rigthShoulder, rigthWrist) < 200:
                start = 1
            elif start and distancia_euc(rigthShoulder, rigthWrist) > 360:
                cnt += 1
                start = 0
